...
public class Updater {
	...
	// Remote file's download link
	private static final String RELEASENOTES_VALUE = "fileUrl";
	...
	// If found update is a bugfix
	private boolean isBugfix = false;
	// notified players
	private List<String> notifiedPlayers;

	/* Collected from Curse API */

	private String versionName;
	private String versionLink;
	private String versionReleaseNotesLink;
	...
	public enum UpdateResult {
		...
		SUCCESS,
		/**
		 * The updater did not find an update, and nothing was
		 * downloaded.
		 */
		NO_UPDATE,
		/**
		 * The server administrator has disabled the updating
		 * system.
		 */
		DISABLED,
		/**
		 * The updater found an update, but was unable to download
		 * it.
		 */
		FAIL_DOWNLOAD,
		/**
		 * For some reason, the updater was unable to contact
		 * dev.bukkit.org to download the file.
		 */
		FAIL_DBO,
		/**
		 * When running the version check, the file on DBO did not
		 * contain a recognizable version.
		 */
		FAIL_NOVERSION,
		/**
		 * The id provided by the plugin running the updater was
		 * invalid and doesn't exist on DBO.
		 */
		FAIL_BADID,
		/**
		 * The server administrator has improperly configured their
		 * API key in the configuration.
		 */
		FAIL_APIKEY,
		/**
		 * The updater found an update, but because of the
		 * UpdateType being set to NO_DOWNLOAD, it wasn't
		 * downloaded.
		 */
		UPDATE_AVAILABLE
	}
	...
	public enum UpdateType {
		...
		BUGFIX_ONLY,
		/**
		 * Don't run a version check, just find the latest
		 * update and download it.
		 */
		NO_VERSION_CHECK,
		/**
		 * Get information about the version and the
		 * download size, but don't actually download
		 * anything.
		 */
		NO_DOWNLOAD
	}
	...
	public Updater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
		...
		this.notifiedPlayers = new ArrayList<String>();
	...
	public List<String> getNotifiedPlayers() {
	...
	/**
	 * Get the latest version's release notes link.
	 *
	 * @return latest version's file link.
	 */
	public String getLatestReleaseNotesLink() {
		this.waitForThread();
		return this.versionReleaseNotesLink;
	}
	...
	private void downloadFile() {
		...
		try {
			...
			while ((count = in.read(data, 0, Updater.BYTE_SIZE)) != -1) {
				...
				if (this.announce && ((percent % 10) == 0) && lastPercent != percent) {
	...
	private void runUpdater() {
		if (this.url != null && (this.read() && this.versionCheck())) {
			...
			if ((this.versionLink != null) && (this.type != UpdateType.NO_DOWNLOAD)) {
				if (this.type == UpdateType.BUGFIX_ONLY && !this.isBugfix) {
...
class Version {
	...
	public boolean isMaintenanceUpdateTo(Version older) {
		return (older.getMajor() == this.major && older.getMinor() == this.minor
				&& older.getMaintenance() < this.maintenance);
	}
